...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/10/16 16:39:43 by fabet             #+#    #+#             */
/*   Updated: 2022/10/16 16:39:44 by fabet            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_simple_cmds	*ft_call_expander(t_tools *tools, t_simple_cmds *cmd)
{
	...
	cmd->str = ft_start_expander(tools, cmd->str);
	start = cmd->redirections;
	while (cmd->redirections)
	{
		if (cmd->redirections->token != LESS_LESS)
			cmd->redirections->str
				= ft_start_expander_str(tools, cmd->redirections->str);
...
int	ft_fork(t_tools *tools, int end[2], int fd_in, t_simple_cmds *cmd)
{
	...
	if (tools->pid[i] < 0)
		ft_error(5, tools);
	if (tools->pid[i] == 0)
		ft_dup_cmd(cmd, tools, end, fd_in);
	i++;
	return (EXIT_SUCCESS);
}

int	ft_check_fd_heredoc(t_tools *tools, int end[2], t_simple_cmds *cmd)
{
	int	fd_in;
...
int	ft_start_executor(t_tools *tools)
{
	...
	while (tools->simple_cmds)
	{
		...
		if (tools->simple_cmds->next)
			pipe(end);
		ft_send_heredoc(tools, tools->simple_cmds);
		ft_fork(tools, end, fd_in, tools->simple_cmds);
		close(end[1]);
		if (tools->simple_cmds->prev)
			close(fd_in);
		fd_in = ft_check_fd_heredoc(tools, end, tools->simple_cmds);
		if (tools->simple_cmds->next)
			tools->simple_cmds = tools->simple_cmds->next;
		else
			break ;
	}
	ft_pipe_wait(tools->pid, tools->pipes);
